...
public enum eRotationMode { SQUAD, PATH_ANGLE, NONE};
...
public class SplineInterpolator : MonoBehaviour
{
	...
	void SetInput()
	{
		...
		if (mRotations != eRotationMode.NONE)
		{
	...
	void Update()
	{
		...
		if (mCurrentTime >= mNodes[mCurrentIdx + 1].Time)
		{
			...
			else
			{
				...
				{
					mState = "Stopped";
					this.SendMessage("EndOfSpline");

					// We stop right in the end point
					transform.position = mNodes[mNodes.Count - 2].Point;

					if (mRotations == eRotationMode.SQUAD) {
						transform.rotation = mNodes[mNodes.Count - 2].Rot;
					} else if (mRotations == eRotationMode.PATH_ANGLE) {
						transform.rotation = GetPathAngleAtTime(mCurrentTime);
					}
	...
	public Quaternion GetPathAngleAtTime(float time) {
		Vector3 dP;

		if (time >= mNodes [mNodes.Count - 1].Time) {
			dP = GetHermiteAtTime(mNodes[mNodes.Count-1].Time) - GetHermiteAtTime(mNodes[mNodes.Count-1].Time-0.02f);
		} else {
			dP = GetHermiteAtTime(time + 0.02f) - GetHermiteAtTime(time);
		}

		Vector3 up = Quaternion.AngleAxis (GetSquadAtTime (time).eulerAngles.z, dP) * Vector3.up;
		return Quaternion.LookRotation (dP, up);
	}

	public Quaternion GetRotationAtTime(float time) {
		if (mRotations == eRotationMode.SQUAD) {
			return GetSquadAtTime (time);
		} else if (mRotations == eRotationMode.PATH_ANGLE) {
			return GetPathAngleAtTime (time);
		} else {
